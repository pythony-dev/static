<?php
    ...
    final class Emails extends Database {
        ...
        public static function create($email, $title, $content) {
            ...
            $query = parent::$pdo->prepare("INSERT INTO " . parent::getPrefix() . "Emails (created, sessionID, userID, email, title, content) VALUES (NOW(), :sessionID, :userID, :email, :title, :content)");
        ...
        public static function count() {
            return parent::$pdo->query("SELECT COUNT(id) AS count FROM " . parent::getPrefix() . "Emails")->fetch()["count"] ?? 0;
        }
        ...
        public static function getEmail($hash) {
            ...
            $query = parent::$pdo->query("SELECT id FROM " . parent::getPrefix() . "Emails ORDER BY ID DESC");

            while($response = $query->fetch()) {
                if(\Static\Kernel::getHash("Email", $response["id"]) == $hash) {
                    $query = parent::$pdo->prepare("SELECT created, email, title, content FROM " . parent::getPrefix() . "Emails WHERE id = :emailID");
